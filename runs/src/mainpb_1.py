...
class EIT:
  def __init__(self, a = a, theta = theta):
    ...
    return
  def domain(self):
    self.ld, self.so, self.sb = x3domain()
  def meshgrid(self):
    self.x, self.y, self.pp = ipb.meshgrid(self.meshgrid_args)
    if self.p == ():
      self.p = self.pp
    else:
      self.p = sg.Pointset(self.p)
  def plot(self):
    plot.plot(self.x, self.y, self.z, 'im')
    self.ld.plot(p=True)
    self.so.plot()
    plt.show(block=False)
  ...
  def solver(self):
    ...
    self.isolver = ipb.solver_init(self.K, self.a, reg, regmet, solver, RHS_fcom=RHS_fcom, RHS_args=RHS_args, BX=self.so.BX, BY=self.so.BY)
  def ipb(self):
  ...
  def test_alpha(self, a=()):
    if a == ():
      self.a = np.array([1e-16 + 1e-16 * k for k in range(10)])
    else:
      self.a = a
    ipb.test_one(self.isolver, self.p, so, self.a) 
    self.z = self.isolver.save_ratio[:, 0]
