...
def method_gap():
  ...
  isolver_gap = ipb.solver_init(R, a, reg, regmet, solver, RHS_fcom=RHS_fcom, RHS_args=RHS_args)
  x, y, pp = meshgrid((-2, 2 , 40))

  (ninv, res, nsolgap) = ipb.iallsols(isolver_gap, pp)
...
def method_NtoD():
  ...
  LL0B = ipb.computeLL0B(ld, so, T=ly.layerpotSD(s=sb, t=so), c=c, testset=0)
  ...
  (ninv, res, nsolgap) = ipb.iallsols(isolver_NtoD, pp)
