...
phi4
# phi3 vs phi4
get_ipython().magic('save my_session_17_2100 1-226')
reload(dpb)
reload(ipb)
L = computeallpsiL0(s, E)
L = ipb.computeallpsiL0(s, E)
s
reload(dpb)
L = ipb.computeallpsiL0(s, E)
L
np.diagflat(s.w).dot(L)
A=np.diagflat(s.w).dot(L)
issym(A)
A - A.T
issym(K)
issym(L)
L = ipb.computeallpsiL0(s, s.S[:,1::])
sum(np.diagflat(s.w).dot(L))
max(abs(sum(np.diagflat(s.w).dot(L))))
L = ipb.computeallpsiL0(s, s.Sw[:,1::])
max(abs(sum(np.diagflat(s.w).dot(L))))
Q = linf.eigmaxpowerw(s=s, s0=np.ones(s.n))
Q = linf.gramschmidtw(s=s, s0=np.ones(s.n))
L = ipb.computeallpsiL0(s, Q[:, 1::])
max(abs(sum(np.diagflat(s.w).dot(L))))
B = Q[:, 1::].T.dot(np.diagflat(s.w).dot(L))
B
issym(B)
